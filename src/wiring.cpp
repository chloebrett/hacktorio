...
Wiring::Wiring(sf::RenderWindow& window) : window(window) {
    ...
    root = new SceneNode(sf::Vector2f(0, 0), sf::Vector2f(SCREEN_WIDTH, SCREEN_HEIGHT),
                         /* onClick= */ nullptr,
                         /* onRender= */ nullptr);

    background = new SceneNode(
        sf::Vector2f(0.0f, 0.0f), sf::Vector2f(SCREEN_WIDTH, SCREEN_HEIGHT),
        /* onClick= */ nullptr,
...
void Wiring::initUi(SceneNode* root, Player* player) {
    ...
    doubleInventoryGridPanel = new Panel(
        ...
        /* size= */
        sf::Vector2f(INVENTORY_WIDTH * 2 + INVENTORY_PADDING * 3,
    ...
    Gui* gui = new Gui(*doubleInventoryGridPanel, *craftingPanel, *entityPanel, *researchPanel,
                       *escapeMenuPanel, *player, *timer, *craftingQueue);
    EntityPlacementManager* entityPlacementManager =
        new EntityPlacementManager(*root, *recipeConfiguration, *gui, *spatialIndex);

    for (int y = 0; y < SCREEN_HEIGHT_CELLS; y++) {
        for (int x = 0; x < SCREEN_WIDTH_CELLS; x++) {
            EmptySpace* emptySpace(new EmptySpace(sf::Vector2f(x * GRID_SIZE, y * GRID_SIZE),
    ...
    resumeButton = new Button(
        ...
        /* textString= */ "Resume",
        /* onClick= */ [&gui](Cursor& cursor) { gui->closeOpenPanels(); });

    sf::RenderWindow& window = this->window;
    quitButton = new Button(
        /* position= */ sf::Vector2f(ESCAPE_MENU_ITEM_PADDING,
                                     ESCAPE_MENU_ITEM_PADDING * 2 + ESCAPE_MENU_ITEM_HEIGHT),
        /* size= */ sf::Vector2f(ESCAPE_MENU_ITEM_WIDTH, ESCAPE_MENU_ITEM_HEIGHT),
        /* fillColor= */ sf::Color(200, 80, 80, 255),
        /* textString= */ "Quit game",
        /* onClick= */ [&window](Cursor& cursor) { window.close(); });

    escapeMenuPanel->addChild(resumeButton);
    escapeMenuPanel->addChild(quitButton);

    inventoryLeft = new InventoryGrid(sf::Vector2f(INVENTORY_PADDING, INVENTORY_PADDING), player);
    inventoryRight =
        new InventoryGrid(sf::Vector2f(INVENTORY_WIDTH + INVENTORY_PADDING * 2, INVENTORY_PADDING),
    ...
    RecipeTab* intermediateProductsTab =
        new RecipeTab(2, RecipeTabType::INTERMEDIATE_PRODUCTS, *recipePanel);
