...
sf::Vector2f SceneNode::getPos() { return pos; }
...
void SceneNode::setSize(sf::Vector2f size) { this->size = size; }

void SceneNode::setOnClick(std::function<void(Cursor&)> onClick) { this->onClick = onClick; }
...
void SceneNode::addParent(SceneNode* parent) { parents.push_back(parent); }
...
void SceneNode::removeParent(SceneNode* parent) {
    ...
    if (it != parents.end()) {
        parents.erase(it);
        // Don't worry about removing the child reference, since we usually call this from
        // removeChild() anyway.
    }
}
...
int SceneNode::getZ() { return z; }

void SceneNode::setZ(int z) { this->z = z; }

bool SceneNode::isInteractive() { return onClick != nullptr; }

bool SceneNode::isVisible() { return visible; }

bool SceneNode::canPlaceItem() { return false; }

bool SceneNode::isTransitivelyVisible() {
    // Returns true if the node itself is directly visible, and at least one of its parents is
    // transitively visible. For the root node, there are no parents to check.
    if (!isVisible()) {
        return false;
    }
...
void SceneNode::setVisible(bool visible) { this->visible = visible; }
