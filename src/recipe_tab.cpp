...
RecipeTab::RecipeTab(
    ...
    RecipeTabType type,
    RecipePanel &panel
) : index(index), type(type), panel(panel), SceneNode(
    /* position= */ sf::Vector2f(RECIPE_TAB_WIDTH * index, 0),
    /* size= */ sf::Vector2f(RECIPE_TAB_WIDTH, RECIPE_TAB_LIST_HEIGHT),
    /* onClick= */ [index, &panel](Cursor &cursor) {
        panel.setSelectedTabIndex(index);
    },
    /* onRender= */ [this](
        SceneNode &node,
